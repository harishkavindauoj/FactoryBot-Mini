...
class GeminiTeacher:
    ...
    def generate_soft_labels_optimized(self, max_workers: int = 5, batch_size: int = 10,
        ...
        # Normalize TTF
        ttf_max = np.max(ttf_array)
        ttf_array = ttf_array / ttf_max

        # Save normalized labels and scale
        np.save('data/processed/ttf_labels.npy', ttf_array)
        with open('data/processed/ttf_max_scale.pkl', 'wb') as f:
            pickle.dump(ttf_max, f)
